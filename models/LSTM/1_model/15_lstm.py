...
def init_lstm(hidden_layer_size, vocab_size, z_size):
    ...
    W_f = np.random.randn(hidden_layer_size, z_size)
    
    # Bias for forget gate
    b_f = np.zeros((hidden_layer_size, 1))

    # Weight matrix (input gate)
    W_i = np.random.randn(hidden_layer_size, z_size)
    
    # Bias for input gate
    b_i = np.zeros((hidden_layer_size, 1))

    # Weight matrix (candidate)
    W_g = np.random.randn(hidden_layer_size, z_size)
    
    # Bias for candidate
    b_g = np.zeros((hidden_layer_size, 1))

    # Weight matrix of the output gate
    W_o = np.random.randn(hidden_layer_size, z_size)
    b_o = np.zeros((hidden_layer_size, 1))

    # Weight matrix relating the hidden-state to the output
    W_v = np.random.randn(vocab_size, hidden_layer_size)
...
#-------------------------------------------------------------------------
#-------------------------------------------------------------------------
def forward(inputs, h_prev, C_prev, params, hidden_size):
    """
    Arguments:
...
def train_LSTM(hidden_layer_size, vocab_size, word_to_idx,  training_set, validation_set):
    ...
    for i in range(num_epochs):
        ...
        for inputs, targets in validation_set:
            ...
            # Forward pass
            z_s, f_s, i_s, g_s, C_s, o_s, h_s, v_s, outputs = forward(inputs = inputs_one_hot, h_prev  = h, C_prev = c, params = params, hidden_size=hidden_layer_size)
