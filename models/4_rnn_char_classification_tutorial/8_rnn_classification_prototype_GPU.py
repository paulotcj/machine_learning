...
import math
#-------------------------------------------------------------------------
def train(rnn, learning_rate, category_tensor, line_tensor, device, is_cuda_available):
...
def execute_train_rnn(rnn, n_iters, all_categories, category_lines, n_letters, all_letters_idx, 
        learning_rate, print_every, plot_every, device, is_cuda_available):
...
result_part5 = execute_part5(
    ...
    n_letters           = result_part1['n_letters'],
    all_letters_idx     = result_part1['all_letters_idx'],
    device              = result_part3['device'],
    is_cuda_available   = result_part3['cuda_available']
)
exit()
...
#-------------------------------------------------------------------------
#-------------------------------------------------------------------------
def evaluate(rnn, line_tensor): # Just return an output given a line - used in the confusion matrix
...
def show_confusion_matrix(rnn, n_categories, all_categories, category_lines, n_letters, all_letters_idx):
    ...
    for i in range(n_confusion):
        ...
        # 3 - make a prediction
        output = evaluate(rnn = rnn,line_tensor = line_tensor)
...
#-------------------------------------------------------------------------
def predict(rnn, input_line, all_categories, n_letters, all_letters_idx, n_predictions=3):
    print(f'\n> {input_line}')

    # set all the requires_grad flags to False. Any operations performed within this 
    #   block will not be tracked for gradients

    #------------------------------------------------
    with torch.no_grad():

        temp_line_tensor = lineToTensor(
            line        = input_line, 
            n_letters   = n_letters, 
            all_letters_idx = all_letters_idx
        )
        output = evaluate(rnn=rnn, line_tensor=temp_line_tensor)

        # Get top N categories
        #  we get the top N (typically 3), and then we get the indexes from where these values are from
        #  for instance: top_v: tensor([[-0.7459, -0.8832, -2.7700]])
        #                top_i: tensor([[6, 0, 9]])
        ...
        for i in range(n_predictions):
            ...
            value = top_v[0][i].item() #since the shape is [1,3]

            category_index = top_i[0][i].item() #shape [1,3]
...
#-------------------------------------------------------------------------
#-------------------------------------------------------------------------
def execute_part7(rnn, all_categories, n_letters, all_letters_idx):
    ...
    for name in names:
        predict( rnn = rnn, input_line = name, all_categories=all_categories, n_letters = n_letters, all_letters_idx = all_letters_idx, n_predictions=3)
